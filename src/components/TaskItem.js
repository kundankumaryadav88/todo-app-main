      ...
      <div className='task-item-left flex gap-3'>
        ...
        {task.isEditable && (
          <form onSubmit={(e) => handleEditSubmitter(e, task.id)}>
            <input
              className='bg-transparent outline-none border-b-2 border-gray-500 pb-1 w-full focus:border-blue-500'
              type='text'
              required
              value={editedText}
        ...
        {!task.isEditable && (
          <p
            className={`group-hover:text-blue-400 ${
              isChecked
                ? "line-through text-gray-500 group-hover:text-blue-600"
                : null
            }`}
          >
            {task.title}
